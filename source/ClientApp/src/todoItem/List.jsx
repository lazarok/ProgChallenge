...
function List({ match }) {    
    ...
    useEffect(() => {
        todoItemService.getAll(todoListId).then(x => setTodoItems(x));
    }, []);
    ...
    return (
        <div>
            <h1>TodoItem</h1>
            <Link to={`${path}/add/${todoListId}`} className="btn btn-sm btn-success mb-2">Add TodoItem</Link>
                    ...
                    {todoItems && todoItems.map(todoItem =>
                        <tr key={todoItem.id}>
                            ...
                            <td style={{ whiteSpace: 'nowrap' }}>
                                <Link to={`${path}/edit/${todoListId}/${todoItem.id}`} className="btn btn-sm btn-primary mr-1">Edit</Link>
